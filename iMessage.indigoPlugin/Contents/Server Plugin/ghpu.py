...
# Modification to get versions and then take to plugin Store via True/False
	...
	def getLatestVersion(self):
		...
		self.logger.info('iMessage: Update Checking. Your Current version is: %s' % currentVersion)

		update = self.getUpdateAlways(currentVersion)

		if (update == None):
			self.logger.info('iMessage: Update Checking. No release available')
			return False
		# assume the tag is the release version
		latestVersion = update['tag_name'].lstrip('v')

		self.logger.info('iMessage: Update Checking. The Github Latest release is: %s' % latestVersion)
		if (ver(currentVersion) >= ver(latestVersion)):
			self.logger.info(u'iMessage: Update Checking. You already have the lastest release.  No update needed.')
			return False
		if (ver(currentVersion) < ver(latestVersion)):
			self.logger.info(u'iMessage: Update Checking. Please visit the Plugin Store to Download')
			return True
